...
3. The plugin generates `llms.txt` and serves it from your site root.
...
= Why is my llms.txt smaller after updating to 8.5.4? =

The plugin now leaves out anything an anonymous visitor cannot read on your site. If you use a membership or access control plugin, your posts and pages are left out by default, because there is no reliable way to tell from the database which of them a visitor can read. The "Excluded content" summary on the settings screen shows what was left out, and you can include individual posts and pages from the editor.

If your file was generated before this update it may have listed content that is not publicly visible, including titles and links. The file is rebuilt when you update. If you have shared or submitted a copy of it anywhere, it is worth replacing that copy.
...
The plugin also writes a copy of the file into your uploads folder, and on hosting where the root directory cannot be written, such as WordPress VIP, that copy is the only one. WordPress serves it at `https://example.com/llms.txt` through a rewrite rule the plugin registers. Rewrite rules are stored and are not rebuilt on their own, so if `/llms.txt` returns a 404 on a site that has already generated the file, use “Clear caches” on the plugin settings screen, or re-save your permalinks under *Settings → Permalinks*.
...
On a very large network, activate from the command line. Network activation clears each site’s stored rewrite rules so every site rebuilds its own, which costs a few milliseconds per site. On networks of several thousand sites that can run past PHP’s time limit in wp-admin, and `wp plugin activate website-llms-txt --network` has no such limit.

= Does it work with LifterLMS? =
No. The two plugins cannot be active on the same site. Both define a PHP class named `LLMS_Generator`, so WordPress stops with a fatal error when the second one loads. This is long-standing and applies to earlier versions too, in both directions. Renaming our class is planned for a future release.
...
• Content that is not publicly visible on your site is no longer included in the generated file. This covers password protected content, content that is not published, and content restricted by membership and access control plugins.
• If you use a membership or access control plugin, your posts and pages are now left out by default, because there is no reliable way to tell from the database which of them a visitor can read. The new "Excluded content" summary on the settings screen shows what was left out, and you can include individual posts and pages from the editor.
• Your existing llms.txt is rebuilt when you update, so anything that should not have been in it is removed.
• Regeneration is faster, and the file is now written in a single step so an interrupted update cannot leave a partial file in place.
